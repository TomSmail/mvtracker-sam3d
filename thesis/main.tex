...
Supervised finetuning consistently degrades performance due to catastrophic forgetting at the pretraining learning rate. A critical methodological finding is that the TAPVid-3D benchmark sequences use Dynamic3DGS depths --- optimization-based reconstructions fitted to all 31 camera views --- as input, effectively providing ground-truth geometry and inflating evaluation results. Evaluation with DUSt3R-estimated depths (online-computable from 4 target views) confirms the inflation: average AJ drops from 55.6\% to 9.1\%. Under these realistic depth conditions, SAM3D mesh augmentation further degrades performance ($-$1.1 AJ points), indicating that the pretrained model treats mesh-injected features as noise when the depth-based point cloud is already noisy.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{lccccccccc}
        ...
        \quad Baseline          & \textbf{11.56} & \textbf{9.55} & \textbf{89.05} & \textbf{7.90} & \textbf{7.89} & \textbf{95.62} & \textbf{7.76} & \textbf{5.43} & 97.15 \\
        \quad + SAM3D mesh      & 9.67 & 9.35 & 95.52 & 7.31 & 7.47 & 96.13 & 6.81 & 5.30 & \textbf{84.96} \\
        \bottomrule
    \end{tabularx}
    \caption{Evaluation with DUSt3R-estimated depth maps (online-computable from 4 target views only). SAM3D mesh augmentation degrades performance on most configurations, reducing average AJ from 9.07 to 7.93 ($-$1.14 points). Bold indicates the better result per metric. All numbers are percentages except ATE (mm).}
    \label{tab:duster_results}
\end{table}

With DUSt3R depth, overall performance drops dramatically compared to Dynamic3DGS (average AJ from 55.63 to 9.07), confirming that the Dynamic3DGS evaluation was inflated by near-perfect geometry. SAM3D mesh augmentation \emph{hurts} under DUSt3R conditions: average AJ decreases from 9.07 to 7.93 ($-$1.14 points), with degradation on all three view configurations for AJ and two of three for ATE. The only improvement is ATE on \texttt{views1\_4\_7\_11} (84.96 vs 97.15\,mm), suggesting that mesh vertices occasionally help localise trajectories in configurations with narrow baselines where DUSt3R depth is least reliable.
...
\textbf{Methodological contribution.} A critical discovery is that the TAPVid-3D benchmark sequences use Dynamic 3D Gaussian Splatting depths as input --- optimization-based reconstructions fitted to all 31 camera views --- which represent near-ground-truth geometry rather than online-computable depth estimates. This inflates baseline performance by $6\times$ (AJ 55.6\% vs 9.1\% with DUSt3R depth). Evaluation with DUSt3R-estimated depths confirms that SAM3D mesh augmentation degrades performance under realistic conditions ($-$1.1 AJ points), indicating that the pretrained model cannot productively incorporate mesh-based features alongside noisy depth.
...
\textbf{Limitations and future work.} The zero-shot augmentation approach depends on the pretrained model's capacity to utilize mesh-based features, which the current MVTracker architecture lacks. The consistent failure across all three depth conditions --- perfect (Dynamic3DGS), estimated (DUSt3R), and absent (zeros) --- indicates that the pretrained feature space fundamentally cannot accommodate mesh-injected points without retraining. Training a model from scratch to fuse depth-based and mesh-based correlation evidence, or designing a depth-agnostic tracking architecture that operates on RGB and parametric body models alone, represent promising directions. Finetuning with an appropriate learning rate ($\sim 5 \times 10^{-6}$) and partial encoder freezing could enable the model to learn to utilise mesh features without catastrophic forgetting. Additional directions include extension to non-human articulated objects and evaluation on broader real-world capture domains beyond Panoptic Studio.
