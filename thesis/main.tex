...
We evaluate on three Panoptic Studio multi-view configurations and report 3D tracking accuracy and occlusion robustness, comparing the baseline depth-only model against the point cloud augmented variant before and after finetuning on human sequences.
...
A preliminary run of 5,000 steps on only four Panoptic training sequences exhibited catastrophic forgetting: AJ on \texttt{panoptic-human-views1\_7\_14\_20} dropped from 56.5 to 36.3, suggesting that limited data diversity caused the backbone to overfit. The primary training run therefore uses all valid sequences from the full 90-sequence pipeline output (approximately 55 after filtering empty-room sequences), trained for 20,000 steps. Empty-room sequences --- those for which the generated \texttt{human\_tracks.npz} contains no valid track frames --- are automatically excluded by the dataset loader.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{lcccccc}
        \toprule
        & \multicolumn{2}{c}{\texttt{views1\_7\_14\_20}} & \multicolumn{2}{c}{\texttt{views27\_16\_14\_8}} & \multicolumn{2}{c}{\texttt{views1\_4\_7\_11}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
        Model & AJ$\uparrow$ & ATE$\downarrow$ & AJ$\uparrow$ & ATE$\downarrow$ & AJ$\uparrow$ & ATE$\downarrow$ \\
        \midrule
        Baseline (depth only)          & 56.48 & 15.72 & 56.66 & 15.57 & 51.55 & 14.54 \\
        PC-aug zero-shot               & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  \\
        PC-aug finetuned (20k steps)   & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  & \textit{TBD}  \\
        \bottomrule
    \end{tabularx}
    \caption{Main results on Panoptic Studio human sequences. AJ = 3D Average Jaccard (\%), ATE = Average Trajectory Error (mm). PC-aug zero-shot uses the pretrained checkpoint with SAM3D vertices added; PC-aug finetuned additionally trains for 20k steps on all $\sim$55 valid Panoptic human sequences.}
    \label{tab:main_results}
\end{table}

\section{Model Training Evaluation}
Model-centric analysis is structured in two stages. First, we compare the kNN semantic bias (zero-shot) against the baseline to characterise its failure mode and confirm that re-ranking without additional geometry provides no measurable benefit. Second, we evaluate the point cloud augmentation approach: comparing the zero-shot augmented model against the baseline, and then comparing pre- and post-finetuning performance to assess how much further benefit is gained from adapting the backbone to the augmented data. Per-metric deltas are reported for each Panoptic view configuration.
...
Limitations include dependence on SAM3D detection availability and occasional sequence-specific coverage gaps (for example, a subject entering the scene mid-sequence). Future work includes investigating vertex subsampling strategies to reduce cloud size, extending the augmentation to non-human articulated objects, and evaluating on broader real-world capture domains beyond Panoptic Studio.
