...
The evaluation protocol described above uses Dynamic3DGS depths, which are fitted to all 31 camera views via multi-view optimization and thus represent near-ground-truth geometry. This is appropriate for establishing an upper bound on performance but does not reflect realistic deployment scenarios where depth must be estimated online from the target views alone. The original MVTracker paper evaluates with DUSt3R --- a learning-based depth estimator that processes only the selected camera views --- to assess robustness to imperfect depth.

To complete the assessment of SAM3D mesh augmentation, evaluation with DUSt3R depth is necessary. DUSt3R depths are noisier and sparser than Dynamic3DGS (gaps at object boundaries, lower confidence in textureless regions), which may create opportunities for mesh vertices to fill gaps and improve correlation quality. This evaluation is identified as immediate future work, with depth generation infrastructure already implemented.
...
\textbf{Limitations and future work.} The zero-shot augmentation approach depends on the pretrained model's capacity to utilize mesh-based features, which the current MVTracker architecture lacks. Training a model from scratch to fuse depth-based and mesh-based correlation evidence, or designing a depth-agnostic tracking architecture that operates on RGB and parametric body models alone, represent promising directions. For the pretrained-model setting, evaluation with DUSt3R depth (pending) will clarify whether mesh augmentation provides value when depth quality is intermediate between Dynamic3DGS (perfect) and zeros (absent). Additional directions include extension to non-human articulated objects and evaluation on broader real-world capture domains beyond Panoptic Studio.
