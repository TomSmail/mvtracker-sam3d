...
\label{ch:intro}
Human-centric 3D point tracking is a particularly challenging variant of general point tracking due to severe self-occlusion, articulated non-rigid motion, and persistent appearance ambiguity across body parts. While the MVTracker framework already leverages multi-view and temporal context, its correlation evidence is derived entirely from depth-based point clouds, which are often incomplete or noisy over human bodies due to reflective clothing, thin limbs, and sensor limitations.

This thesis investigates whether directly enriching the 3D point cloud with human mesh geometry can improve track consistency and robustness under these conditions. We integrate SAM-3D-Body outputs into MVTracker by augmenting the depth-based point cloud with SMPL mesh vertices, providing explicit geometric coverage of the human body surface where depth maps are unreliable. We also build a dedicated human data engine that provides dense trajectory supervision from Panoptic Studio sequences.
...
\chapter{Background}
\label{ch:background}
\section{Multi-View Track Any Point (MV-TAP)}
The objective of point tracking is to estimate a query point's trajectory and visibility through time. MV-TAP \cite{koo2025mvtaptrackingpointmultiview} demonstrates that combining multi-view context with temporal modelling improves tracking robustness beyond methods that treat these axes separately.

\section{MVTracker Architecture}
MVTracker back-projects RGBD observations into 3D point clouds and retrieves local correlation evidence through kNN-based lookup. A temporal-spatial transformer then iteratively refines track coordinates and visibility predictions. In the baseline design, neighbour retrieval is purely Euclidean in 3D space, which is effective for generic scenes but does not explicitly encode human-part semantics.
...
\section{SAM3D and MHR Priors}
\label{sec:sam3d}
SAM-3D-Body provides per-frame human mesh fits and keypoints, including 70 MHR joints per detected person. These joints define a compact semantic scaffold over the body that can be used to measure whether two candidate points are likely to correspond to similar anatomical regions, even under viewpoint changes or temporary occlusions.
...
SAM-3D-Body produces approximately 6,890 SMPL mesh vertices per detected person per frame in world coordinates. These are appended to the existing depth-based point cloud ($\sim$57k points) at each pyramid level, representing a $\sim$12\% increase in cloud size with modest computational overhead.
...
We implemented a multi-stage offline data engine to produce MVTracker-compatible human trajectory supervision from Panoptic Studio data.
