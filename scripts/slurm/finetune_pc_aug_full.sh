#!/bin/bash
#SBATCH --job-name=finetune-pc-aug-full
#SBATCH --nodes=1
...
echo "=== Fine-tuning MVTracker + SAM3D PC-Aug on full dataset (20k steps) ==="
...
# Finetune with SAM3D pointcloud augmentation, all valid sequences, 20k steps
PYTHONPATH=$DIR python -m mvtracker.cli.train \
  +experiment=mvtracker_human_finetune_pc_aug_full
