#!/bin/bash
#SBATCH --job-name=eval-dust-repl
#SBATCH --nodes=1
...
echo "=== DUSt3R + SAM3D replace: mesh replaces noisy depth in body regions ==="
...
PYTHONPATH=$DIR python -m mvtracker.cli.eval \
  +experiment=mvtracker_human_eval_duster_pc_aug_replace
