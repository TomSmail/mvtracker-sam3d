#!/bin/bash
#SBATCH --job-name=eval-zerodepth-pv
#SBATCH --nodes=1
...
echo "=== Zero-depth + per-view SAM3D (500v): no averaging across views ==="
...
PYTHONPATH=$DIR python -m mvtracker.cli.eval \
  +experiment=mvtracker_human_eval_zerodepth_perview_sparse
