...
class MVTracker(nn.Module):
    def __init__(
            ...
            pc_aug_per_view_features: bool = False,
    ):
        super().__init__()
        ...
        self.pc_aug_per_view_features = pc_aug_per_view_features
    ...
    def forward_iteration(
        ...
        for lvl in range(self.corr_n_levels):
            ...
            if self.use_sam3d_pointcloud_augmentation and sam3d_vertices is not None:
                if self.pc_aug_per_view_features:
                    # Use per-view features (no averaging across views)
                    from mvtracker.models.core.model_utils import augment_pointcloud_with_mesh_vertices_per_view
                    augment_fn = augment_pointcloud_with_mesh_vertices_per_view
                    augment_kwargs = dict(
                    ...
                    # Use averaged features (original approach)
                    augment_fn = augment_pointcloud_with_mesh_vertices
                    augment_kwargs = dict(
                        pointcloud_xyz=pc_xyz,
                        pointcloud_fvec=pc_fvec,
                        mesh_vertices=sam3d_vertices,
                ...
                if pc_valid is not None:
                    pc_xyz, pc_fvec, pc_valid = augment_fn(**augment_kwargs)
                else:
                    augment_kwargs.pop('pointcloud_valid')
                    pc_xyz, pc_fvec = augment_fn(**augment_kwargs)
